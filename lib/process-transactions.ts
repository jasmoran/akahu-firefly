...
function assetIdForAccount (akahuAccountId: string): number {
...
export function processTransactions (transactions: EnrichedTransaction[]): TransactionSplitStore[] {
  const processed = transactions.map(transaction => {
    ...
    if (transaction.amount < 0) {
      ...
      fireflyTrans.source_id = assetIdForAccount(transaction._account).toString()
      fireflyTrans.destination_id = 'expense account' // TODO
    } else {
      fireflyTrans.type = 'deposit'
      fireflyTrans.source_id = 'revenue account' // TODO
      fireflyTrans.destination_id = assetIdForAccount(transaction._account).toString()
    }
    ...
    fireflyTrans.category_name = transaction?.category?.groups?.personal_finance?.name
